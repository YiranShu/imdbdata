...
This part we will analyze the relationship between directors and genre types, which contain features of some directors. Do you want to know is there any preference of each director? You will figure out in the following analysis. 
...
ggplot(data=top_10, aes(x=reorder(primaryName, count), y=count)) + geom_col(fill="cornflowerblue") + coord_flip() +
...
inner_join(title.genres, title.crew, by="tconst") %>%
  ...
  ggplot(aes(x=reorder(genres, count), y=count)) + geom_col(fill="cornflowerblue") + coord_flip() +
...
inner_join(title.genres, title.crew, by="tconst") %>%
  ...
  ggplot(aes(x=primaryName, y=genres, fill=rating)) + geom_tile() +
