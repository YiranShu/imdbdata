...
ggplot(data=top_10, aes(x=reorder(primaryName, count), y=count)) + 
       geom_col(fill="cornflowerblue") + 
       coord_flip() +
       labs(x="Director", y="Number of movies", 
       title="Top 10 directors (Action)")
```
...
In this part, the goal I want to achieve is to figure out whether a director like Godfrey Ho made a lot of 'Action' moives has higher ratings in 'Action' movies than other genres or not. The more 'Action' movies one director made, the higher rating he could get in 'Action' movies? Here I also use two types of ratings, average rating and weighted rating, to avoid unfairness. 

In the average rating graph, it is obvious that there is no apparent relationship between the number of movies a director made and the rating he got. You can see director Godfrey Ho has about average rate 5 in all genres, which is not a high rating. In 'Action' movies, He also did not have high ratings, which implies that it does not mean the more 'Action' moveis you made, the higher rating you can get. 
...
inner_join(title.genres, title.crew, by="tconst") %>%
  filter(!is.na(directors)) %>% 
  filter(directors %in% top_10$directors) %>%
  inner_join(title.ratings, by="tconst") %>%
   inner_join(name.basics %>% select(nconst, primaryName), by=c("directors"="nconst")) %>%
  group_by(genres, primaryName) %>% summarise(rating=mean(averageRating)) %>%
  ggplot(aes(x=primaryName, y=genres, fill=rating)) + 
         geom_tile() +
